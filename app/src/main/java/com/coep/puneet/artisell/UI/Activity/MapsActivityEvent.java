...
public class MapsActivityEvent extends FragmentActivity implements OnMapReadyCallback
{
    ...
    public void onMapReady(GoogleMap googleMap)
    {
        ...
        for(int i = 0 ; i < manager.eventList.size(); i++) {
            mCoords.add(new LatLng(manager.eventList.get(i).getCoordinates().getLatitude(), manager.eventList.get(i).getCoordinates().getLongitude()));
            mMap.addMarker(new MarkerOptions().position(mCoords.get(i)).title(manager.eventList.get(i).getEventName()).snippet("-" + manager.eventList.get(i).getStartDate().toLocaleString()));
        }
